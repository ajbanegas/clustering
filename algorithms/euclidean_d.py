...
for i, query in enumerate(query_list):
	...
	# classical distances
	d_euc, elems_euc = distance(X, query)

	# save compund indexes for plotting
	save_items(f"elements/euclidean-q-{i+1}.txt", elems_euc)
	save_items(f"compounds/euclidean-q-{i+1}.txt", df.iloc[elems_euc].index.to_numpy(), fmt="%s")

	print('Compounds:', df.iloc[elems_euc].index.to_numpy())
	print()
