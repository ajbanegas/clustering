...
def plot_kmeans(clf, X):
	...
	plt.title('K-Means')
	plt.tight_layout()
	plt.savefig('images/kmeans-clusters.png')
	plt.close()
...
clf = KMeans(n_clusters=NCLUSTERS, random_state=SEED).fit(X)
save_model(clf, "models/kmeans.joblib")
...
for i, query in enumerate(query_list):
	...
	save_items(f"elements/kmeans-q-{i+1}.txt", elems_km)
	save_items(f"compounds/kmeans-q-{i+1}.txt", df.iloc[elems_km].index.to_numpy(), fmt="%s")

	print('Compounds:', df.iloc[elems_km].index.to_numpy())
	#compute_metrics([], [], y_kmeans)
	print()
